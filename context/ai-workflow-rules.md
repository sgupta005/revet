...
| **6. Chat History** | `ChatThread` table (ownership: `thread_id → user + repo`); upsert on new thread in `/chat`; `GET /repos/{owner}/{repo}/chat/threads` (list); `GET /chat/threads/{thread_id}` (messages read back from checkpointer). |
| **7. PR Review** | Multi-agent fan-out graph → posted review + activity row. |
| **8. Issue Analysis** | Agentic-RAG graph → comment + activity row. |
| **9. Auto-PR** | Plan→generate→commit graph → PR + link comment (label-gated). |
| **10. Evals** | Golden datasets + `langsmith.evaluate` + LLM-as-judge for review & chat. |
| **11. Polish** | Celery retries/backoff + dead-letter; structured logging; Flower; README + diagram. |
